...
clicks = []
vid_capture = cv2.VideoCapture("http://192.168.1.98:5000") # CHANGE DEPENDING ON IP OF CAMERAS: http://192.168.1.98:5000

while vid_capture.isOpened():
  ret, frame = vid_capture.read()
  cv2.imshow('video', frame)
  if cv2.waitKey(1) == ord('p'):
    cv2.imwrite('tempLength.png', frame)
    cv2.destroyAllWindows()
    vid_capture.release()
    break

img = cv2.imread('tempLength.png', cv2.IMREAD_COLOR)
...
while True:
    ...
    if len(clicks) == 4:
        reference = abs(clicks[0][0] - clicks[1][0]) # fkn brainlet error, this only takes the x distance between the two clicks. If ur reusing this code, use pyth. thm to find distance. - James 2023
        ratio = 40. / reference
